...
class Augmentators:
    ...
    @staticmethod
    def _random_sharpening(image, u=0.5):
        if np.random.random() < u:
            return cv2.filter2D(image, -1, Augmentators.sh_filter)
        return image

    @staticmethod
    def _random_shift_scale_rotate(image, mask,
    ...
    @staticmethod
    def _random_horizontal_flip(image, mask, u=0.5):
        if np.random.random() < u:
    ...
    @staticmethod
    def _random_vertical_flip(image, mask, u=0.5):
        if np.random.random() < u:
